...
public class TestController : Controller
{
    ...
    public JsonResult MakeCredentialOptionsTest([FromBody] TEST_MakeCredentialParams opts)
    {
        ...
        var existingKeys = _demoStorage.GetCredentialsByUser(strServer, iPort, iDB, vKey).Select(c => c.Descriptor).ToList();
    ...
    public IActionResult AssertionOptionsTest([FromBody] TEST_AssertionClientParams assertionClientParams)
    {
        ...
        // 1. Get user from DB
        var user = _demoStorage.GetUser(strServer, iPort, iDB, vKey);
        if (user == null)
            return NotFound("username was not registered");

        // 2. Get registered credentials from database
        var existingCredentials = _demoStorage.GetCredentialsByUser(strServer, iPort, iDB, vKey).Select(c => c.Descriptor).ToList();
