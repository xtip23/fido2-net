...
public class ApiFidoController : Controller
{
    ...
    public ApiFidoController(IFido2 fido2, IConfiguration configuration)
    {
        _fido2 = fido2;
    ...
    /// <summary>
    /// đăng ký
    /// tạo tùy chọn thông tin xác thực
    /// </summary>
                                            ...
                                            [FromForm] string userVerification)
    {
        try
        {
            ...
            var existingKeys = DemoStorage.GetCredentialsByUser(username).Select(c => c.Descriptor).ToList();
    ...
    public async Task<JsonResult> MakeCredential([FromBody] AuthenticatorAttestationRawResponse attestationResponse, CancellationToken cancellationToken)
    {
        try
        {
            ...
            IsCredentialIdUniqueToUserAsyncDelegate callback = static async (args, cancellationToken) =>
            {
                ...
                var users = await DemoStorage.GetUsersByCredentialIdAsync(args, cancellationToken);
                if (users.Count > 0)
                    return false;
             ...
             DemoStorage.AddCredentialToUser(options.User, new StoredCredential
            {
                ...
                Type = success.Result.Type.ToString(),
    ...
    public ActionResult AssertionOptionsPost([FromForm] string username, [FromForm] string userVerification)
    {
        ...
        { 
            ...
            if (!string.IsNullOrEmpty(username))
            {
                ...
                existingCredentials = DemoStorage.GetCredentialsByUser(username).Select(c => c.Descriptor).ToList();
            }
    ...
    public async Task<JsonResult> MakeAssertion([FromBody] AuthenticatorAssertionRawResponse clientResponse, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var creds = DemoStorage.GetCredentialById(clientResponse.Id, clientResponse.username) ?? throw new Exception("Unknown credentials");

            // 3. Get credential counter from database
            var storedCounter = creds.SignCount;
            string user = clientResponse.username;
            ...
            IsUserHandleOwnerOfCredentialIdAsync callback = static async (args, cancellationToken) =>
            {
                ...
                //var storedCreds1 = DemoStorage.GetCredentialsByUser(clientResponse.username).Select(c => c.Descriptor).ToList();
            ...
            DemoStorage.UpdateCounter(res.CredentialId, res.SignCount, clientResponse.username);
